...
```{r}
plants_all <- read_rds(here::here("data/TRY_traits_updated_names.rds"))
...
```{r}
plants_pca3 <- map_df(plants_pca2, ~log10(.) %>% scale() %>% as.vector)
...
```{r}
plants_all$SSD[plants_all$SSD <= 0] <- NA
plants_processed <- plants_all %>% 
  select(-phot_path) %>% 
