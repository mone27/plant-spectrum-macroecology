...
```
```{r}
groot <- read_csv("big_groot_correct_names.csv") %>% select(-X1) #removing index
try <- read_rds("try_corr_names.rds") %>%  select(-phot_path)
```

top interesting traits by data availability
```{r}
groot_traits <- c("Specific_root_length","Root_N_concentration","Mean_Root_diameter","Root_tissue_density")
```
