...
```{r}
library(tidyverse)
gift <- read_rds("paper/20210625_GIFT_traits.rds")
```
...
```{r}
sel_traits <- top_7[!top_7 %in% c("Life_form_1", "Growth_form_1")]
```
...
```{r}
pcoa1 <- pcoa(gift_gower_dist)
```
```{r}
biplot.pcoa(pcoa1, gift_pcoa)
```
...
```{r}
pcoa2 <-cmdscale(gift_gower_dist, eig=TRUE, x.ret=TRUE)
